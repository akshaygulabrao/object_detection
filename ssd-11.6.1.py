...
python3 ssd-11.6.1.py --restore_weights=saved_models/ResNet56v2_4-layer_weights-200.h5 -t -b=4

2)  ResNet50 (v2) backbone.
    Evaluate:

python3 ssd-11.6.1.py -e --restore_weights=saved_models/ResNet56v2_4-layer_weights-200.h5 \
...
class SSD:
    ...
    def build_dictionary(self):
        ...
        """
        # train dataset path
        path = os.path.join(self.args['data-path'],
                            self.args['train-labels'])
    ...
    def train(self):
        ...
        save_dir = os.path.join(os.getcwd(), self.args['save-dir'])
    ...
    def evaluate_test(self):
        ...
        for key in keys:
            ...
            image_file = os.path.join(self.args['data-path'], key)
            image = skimage.img_as_float(imread(image_file))
            image, classes, offsets = self.detect_objects(image)
            # perform nms
...
if __name__ == '__main__':
    ...
    args = {'height': 512,
            ...
            'batch_size': 1,
            'save-dir': 'my_weights',
            # 'restore_weights': 'ResNet56v2-2layer-extra_anchors-easy-020.h5',
            'restore_weights': '',
            'improved_loss': False,
            'smooth_l1': False,
            'normalize': False,
            'threshold': 0.8,
            'test-labels': 'easy.csv',
            'train-labels': 'easy.csv',
            'layers': 2,
            'epochs': 40,
            'train': True,
            'image-file': 'easy\easy.jpg',
            'class_threshold': 0.3,
            'iou_threshold': 0.2,
            'soft_nms': False,}
    ssd = SSD(args)
    ...
    if args['restore_weights'] != '':
        ssd.restore_weights()
        if args['evaluate']:
            if args['image-file'] is None:
                ssd.evaluate_test()
            else:
                ssd.evaluate(image_file=args['image-file'])
